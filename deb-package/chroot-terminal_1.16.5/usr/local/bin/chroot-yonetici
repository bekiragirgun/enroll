...
def get_os_info():
    """Host işletim sistemi bilgilerini /etc/os-release üzerinden oku."""
    info = {"ID": "debian", "VERSION_ID": "12"} # Varsayılan
    try:
        ...
        log.warning(f"⚠️ OS tespiti yapılamadı: {e}")
    return info

OS_INFO = get_os_info()
HOST_OS = OS_INFO.get("ID", "debian").lower()
log.info(f"🖥️  Host OS detected: {HOST_OS} (Version: {OS_INFO.get('VERSION_ID', '?')})")
...
def _fix_su_binary(chroot_path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Dağıtıma göre uygun paket URL'lerini belirle
        if HOST_OS == "debian":
            mirrors = [
            ...
            # Ubuntu 24.04 (Noble) util-linux versiyonu
            mirrors = [
                f"http://archive.ubuntu.com/ubuntu/pool/main/u/util-linux/util-linux_2.39.3-9ubuntu6_{arch}.deb",
                f"http://security.ubuntu.com/ubuntu/pool/main/u/util-linux/util-linux_2.39.3-9ubuntu6_{arch}.deb",
            ]
        else:
            mirrors = [f"http://deb.debian.org/debian/pool/main/u/util-linux/util-linux_2.38.1-5+deb12u3_{arch}.deb"]
...
def _fix_etc_hosts(chroot_path):
    ...
    try:
        hostname = socket.gethostname().strip() or "chroot-host"
    except Exception:
        hostname = "chroot-host"
...
def repair_system_pty():
    ...
    # 1. /dev/pts mount edilmiş mi ve ptmxmode doğru mu kontrol et.
    # devpts mevcut olup ptmxmode=000 ile mount edilmişse de remount gerekir —
    # LXC CT 991'de buradaki regresyon PTY allocation (5,2) hatasına yol açıyor.
    try:
        mounts = subprocess.run(["mount"], capture_output=True, text=True).stdout
        devpts_line = next(
            (ln for ln in mounts.splitlines() if "on /dev/pts type devpts" in ln),
            None,
        )
        remount_gerek = False
        if devpts_line is None:
            log.warning("⚠️ /dev/pts devpts olarak mount edilmemiş! Düzeltiliyor...")
            remount_gerek = True
        else:
            m = re.search(r"ptmxmode=(\d+)", devpts_line)
            ptmxmode = m.group(1) if m else None
            if ptmxmode not in ("620", "666"):
                log.warning(
                    f"⚠️ /dev/pts ptmxmode hatalı (tespit edilen: {ptmxmode}). Remount ediliyor..."
                )
                remount_gerek = True
            else:
                log.info(f"✅ /dev/pts zaten doğru mount edilmiş (ptmxmode={ptmxmode}).")
        if remount_gerek:
...
def setup_template():
    ...
    # Şablon için OS bazlı mirror ve distro seçimi
    distro = "bookworm" if HOST_OS == "debian" else "noble" if HOST_OS == "ubuntu" else "bookworm"
    mirror = "http://deb.debian.org/debian" if HOST_OS == "debian" else "http://archive.ubuntu.com/ubuntu"
    keyring = "debian-archive-keyring" if HOST_OS == "debian" else "ubuntu-keyring"
    ...
    if not STUDENT_TEMPLATE.exists():
        ...
        log.info(f"⬇️  mmdebstrap ile {distro} template indiriliyor ({mirror})...")
        _run([
            ...
            f"--include=gnupg,{keyring},gpgv,ca-certificates",
            distro,
...
"""
        ...
        repo_content = "deb http://deb.debian.org/debian bookworm main"
...
def create_student_chroot(username, real_name=""):
    ...
    # plocate'in updatedb'si /proc'a ihtiyaç duyar (linkat hatası olmaması için
    # mount noktalarını doğru okuması gerek). Geçici proc mount açıp kapat.
    proc_path = student_path / "proc"
    proc_path.mkdir(parents=True, exist_ok=True)
    proc_mounted = subprocess.run(
